...
-- 游戏内置图标材质路径（不使用外部 HTTP 加载 PNG）
-- - 普通击杀: content/ui/materials/hud/interactions/icons/enemy
-- - 爆头击杀: content/ui/materials/hud/interactions/icons/enemy_priority
-- - 爆头圆环特效: content/ui/materials/backgrounds/scanner/scanner_drill_wireframe_small
...
-- 为每个slot创建widget对
-- 使用游戏内置材质路径作为 value（直接通过 content.icon 动态切换）
for i = 1, MAX_SLOTS do
...
HudKillIcon.update = function(self, dt, t, ui_renderer, render_settings, input_service)
    ...
    local display_duration = (tonumber(KI:get("kill_icon_duration")) or 20) / 10  -- 存储值除以10得到秒数
